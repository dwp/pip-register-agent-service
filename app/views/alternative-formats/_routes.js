...
const setWrittenFormat = (req, res) => {
  ...
  switch (altFormats.written.format) {
    ...
    case 'audio' :
      ...
      break;
    case 'irish-sign-language-video' :
      validKeys = ['britishSignLanguageVideoFormat', 'email']
      // relativePath = "../"
      break;
    case 'pdf' :
  ...
  checkAnswersOrDefault(patternRootPath + '/spoken-format', res)
...
const setSpokenFormat = (req, res) => {
  ...
  checkAnswersOrDefault(patternRootPath + '/other-help', res)
}
 ...
 * POST: british-sign-language-video - redirect email question if mpeg else redirect to spoken format
 */
router.post(patternRootPath + '/irish-sign-language-video', (req, res) => { 
  
  if (altFormats.written.options['irishSignLanguageVideoFormat'] == 'mpeg') {
...
router.post(patternRootPath + '/other-help', (req, res) => { 
  ...
  } else if (altFormats.otherHelp.required == "no") {
    res.redirect(patternRootPath + '/start-info') // This will be the are you getting disability living allowance for children page
}
})
